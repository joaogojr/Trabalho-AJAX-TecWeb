...
xhr.onreadystatechange = function(){
	if (xhr.readyState === 4){
		...
		for (var i=0; i<dado.length; i++){
				...
				conteudoHTML += item_produto(dado[i].id,dado[i].nome,dado[i].valor,dado[i].imagem,dado[i].descricao);
		}
...
xhr.open('GET', 'service.php?acao=listar_produtos&id_cat=0');
xhr.send();
...
xhr2.open('GET', 'service.php?acao=listar_categorias');
xhr2.send();		
...
select_element.onchange = function() {
	...
	xhr.open('GET', 'service.php?acao=listar_produtos&id_cat='+value);
	xhr.send();	
...
//Exemplo de Template Simples para ITEM DE PRODUTO

var item_produto = function(id, nome, valor, imagem, descricao){
...
function delete_row(e,id)
{
	...
	xhr.onreadystatechange = function(){
		if (xhr.readyState === 4){
			...
			if(dado == 1){		e.parentNode.parentNode.parentNode.parentNode.removeChild(e.parentNode.parentNode.parentNode);
			}else{
...
function enviar(){
	...
	if(nome == "" || url == "" || descricao == "" || preco == "" || categoria == 0){
	   alert("Todos os campos devem ser preenchidos!");
	}else{
   
		var http = new XMLHttpRequest();
		var params = 'nome='+nome+'&url='+url+'&descricao='+descricao+'&preco='+preco+'&categoria='+categoria+'';
		...
		http.onreadystatechange = function() {
			if(http.readyState == 4 && http.status == 200) {
				...
				if(dado != 0){
					...
					var conteudoHTML = item_produto(dado,nome,preco,url,descricao);
